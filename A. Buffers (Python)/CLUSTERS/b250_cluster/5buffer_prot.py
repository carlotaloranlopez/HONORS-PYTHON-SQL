# --------------------------------------------------------------------------------------------
# This script removes protected land from all buffers (including b0) in the 250 cluster.
# It outputs shapefiles b250_cluster_b*_ring_prot, saved to each buffer folder.
# --------------------------------------------------------------------------------------------
...
for b in buffers:
    ...
    input_vector = (
        f"{cd}b250_cluster_{b}/b250_cluster_{b}.shp" if b == "b0"
        else f"{cd}b250_cluster_{b}/b250_cluster_{b}_ring.shp"
    )

    fixed_vector = (
        f"{cd}b250_cluster_{b}/b250_cluster_{b}_fixed.shp" if b == "b0"
        else f"{cd}b250_cluster_{b}/b250_cluster_{b}_fixed_ring.shp"
    )
    
    output_vector = (
        f"{cd}b250_cluster_{b}/b250_cluster_{b}_prot.shp" if b == "b0"
        else f"{cd}b250_cluster_{b}/b250_cluster_{b}_ring_prot.shp"
    )
